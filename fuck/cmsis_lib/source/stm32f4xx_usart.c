...
void usart_putint(USART_TypeDef* USARTx, int value, int radix)	// wysy³a na port szeregowy tekst
{
	char string[17];			// bufor na wynik funkcji itoa
	itoa(value, string, radix);		// konwersja value na ASCII
	usart_puts(USARTx, string);			// wyœlij string na port szeregowy
}

void usart_putlong(USART_TypeDef* USARTx, int value, int radix)	// wysy³a na port szeregowy tekst
{
	char string[17];			// bufor na wynik funkcji itoa
	ltoa(value, string, radix);		// konwersja value na ASCII
	usart_puts(USARTx, string);			// wyœlij string na port szeregowy
}



void usart_putfloat(USART_TypeDef* USARTx, double value){    //wysy³a na port szeregowy liczbê float z dok³adnoci¹ do 3 miejsc po przecinku

	char buf[30];
//	char *str = buf;
//	float fvar = 0.00488 ;  // always works
//	                ftoa(fvar, 2, str);
//	                lcd_locate(0,0);
//	                lcd_str(str);
